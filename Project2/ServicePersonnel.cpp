...
List<Personnel^>^ ServicePersonnel::SelectAllPersonnel() {
	...
	for each (DataRow ^ row in ds->Tables[0]->Rows)
	{
		...
		if (row->IsNull("Num_Rue"))
			...
			a->setNumRue((int)row["Num_Rue"]);

		a->setNomrue((String^)row["Nom_Rue"]);
		a->setComplementAdr((String^)row["Complement_Adresse"]);
		...
		p->setDateNaissance((DateTime)row["Naissance_Personnel"]);
...
void ServicePersonnel::UpdatePersonnel(Personnel^ personnel) {
	...
	String^ query = "UPDATE Personnel SET Nom_Personnel = '" + personnel->getNomPersonnel() + "', Prenom_Personnel = '" + personnel->getPrenomPersonnel() + "', Naissance_Personnel = '" + personnel->getDateNaissance().ToString("yyyy-MM-dd") + "', Date_Embauche = '" + personnel->getDateEmbauche().ToString("yyyy-MM-dd") +
